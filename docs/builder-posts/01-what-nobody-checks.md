...
the medicine drawer — against live US federal safety data, and it wakes you only when one of them
becomes dangerous.

This post is about why that is a real problem and why it needs an agent rather than an app. The
second post in this series is about how it is built on the Strands Agents SDK. The third is about the
...
Here is a real example that I did not have to hunt for. In May 2026, Walmart recalled Mainstays
9-drawer fabric dressers — about 165,000 of them, for "Risk of Serious Injury or Death from Tip-Over
and Entrapment". In August 2026, CPSC re-announced the same recall, because Walmart had distributed
recalled dressers *after* the recall, through liquidators, to consumers. Two recall numbers, 26522
and 26726, for the same object. If you bought one of those dressers from a liquidator, you are on
nobody's list at all, and the notice about it is in somebody else's letterbox.
...
The obvious product here is a website with a search box. Type your make and model, get your recalls.
Those exist. They do not solve the problem, and the reason is simple: an app is a thing you have to
remember to open.
...
An intake agent with a forced output schema turns that into things it can watch, and it is made to
...
model at all — it goes to NHTSA's own vPIC decoder and comes back as fact.
...
the house — NHTSA recalls, NHTSA owner complaints, the CPSC product recall corpus, and openFDA
enforcement reports — and records every question it asked: the URL, whether the source answered, how
many rows came back, how long it took. A source that times out is recorded as **unchecked**, never as
clear. Silence is not safety, and a watch that quietly treated a timeout as "nothing wrong" would be
...
The pass read **4,336 government records across 6 source calls in 166.6 seconds**. It wrote five
...
*is* the work. And what the person answers is stored in their own words as a standing rule, so it is
never asked twice.

I have come to think that "I cannot tell, and here is the one thing that would settle it" is the most
undervalued output an agent can produce. Everything in the industry pushes toward producing an
answer. In a safety product, an honest halt is worth more than a confident answer, and it has to be
made of real machinery — persisted state, a resumable run — rather than a hedge in the prose.
...
key. The demo house is four ordinary objects and every finding on it is a real government record you
can look up yourself. The source is MIT at **https://github.com/shariqazeem/vigil**.

Next post: how this is built on the Strands Agents SDK — the graph with conditional edges, the
tool-raised interrupt and its resume, hooks as enforcement rather than instruction, and the
completeness-gate pattern that turned out to matter more than any of them.
